...
parser.add_argument(
    "--output_root",
    default=None,
    help="Root directory for output NPZ files. Defaults to <SAR_sea_ice_dataset>/VECTOR_FIELDS_24h_pairs_wBackwardPastDrift/<pol_folder>",
)
parser.add_argument(
    "--data_paths_config",
...
if args.output_root is not None:
    ...
    output_root = Path(_paths["SAR_sea_ice_dataset"]) / f"VECTOR_FIELDS_24h_pairs_wBackwardPastDrift/{pol_folder}"
logger = init_logging()
...
for idx in df.index:
    ...
    # Compute !BACKWARD! PAST drift: t to t_minus_24
    vf_past = compute_vector_field(f_center, f_minus, pol_mode)
    ...
    if vf_past is not None and vf_future is not None:
        ...
        outpath_past = out_dir / f"{Path(f_minus).stem}__{Path(f_center).stem}_backward_past.npz"
