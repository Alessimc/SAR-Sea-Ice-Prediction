...
Pixel size is set to 100 m by default.

Supports pol_mode:
    "HV"     - HV only  (default, original behaviour)
    "HH"     - HH only
    "HV+HH"  - run FT+PM separately on HV and HH, then combine
"""
...
def preprocess_tiff_to_fakenansat(
    ...
    """
    Legacy helper for HV-only processing. Kept for compatibility.
    """
...
def load_pols_uint8(filename):
    """
    Load both HH and HV as uint8 images with their recommended dB ranges.

    Returns
    -------
    imgs : dict
        {
          "HH": uint8 2D np.array (scaled from [-18,-8] dB),
          "HV": uint8 2D np.array (scaled from [-28,-14] dB)
        }
    """
    da = rxr.open_rasterio(filename)
    mask = da.isel(band=3).data

    def extract_pol(idx, vmin_db, vmax_db):
        pol = da.isel(band=idx).data.astype("float32")
        pol[mask == 0] = np.nan
        pol[pol <= 0] = np.nan
        pol_db = 10.0 * np.log10(pol)
        return get_uint8_image(pol_db, vmin_db, vmax_db, pmin=10, pmax=99)

    imgs = {}
    # HH: recommended [-18, -8] dB
    imgs["HH"] = extract_pol(0, -18.0, -8.0)
    # HV: recommended [-28, -14] dB
    imgs["HV"] = extract_pol(1, -28.0, -14.0)
...
# HIGH-LEVEL WRAPPER FOR TIFFS (WITH MULTI-POL SUPPORT)
...
class SeaIceDriftFromTiff(object):
    ...
    def __init__(self, file1, file2,
        ...
        self.pol_mode = pol_mode.upper()

        # Load both pols as uint8 images
        self.imgs1 = load_pols_uint8(file1)
        self.imgs2 = load_pols_uint8(file2)

        # For backward compatibility, expose HV FakeNansat
        # (so sid.n1[1] etc still work with HV)
        self.n1 = FakeNansat(self.imgs1["HV"], pixel_size_m=pixel_size_m, timestamp=time1)
        self.n2 = FakeNansat(self.imgs2["HV"], pixel_size_m=pixel_size_m, timestamp=time2)

    # ---------- internal helpers ----------

    def _make_nansat_for_pol(self, pol):
        """Create FakeNansat objects for given polarization."""
        img1 = self.imgs1[pol]
        img2 = self.imgs2[pol]
        n1 = FakeNansat(img1, pixel_size_m=self.pixel_size_m, timestamp=self.time1)
        n2 = FakeNansat(img2, pixel_size_m=self.pixel_size_m, timestamp=self.time2)
        return n1, n2

    def _get_drift_FT_for_pol(self, pol,
                              max_speed=0.5, max_drift=8000.0,
                              ratio_test=0.7, psi=200, verbose=False,
                              **kwargs):
        """
        Run FT on one polarization, return pixel-level displacements.
        """
        n1, n2 = self._make_nansat_for_pol(pol)
        ...
        # convert to pixel displacements via synthetic lat/lon
        lon1, lat1 = n1.transform_points(x1, y1, 0)
        lon2, lat2 = n2.transform_points(x2, y2, 0)
        c1, r1 = n1.transform_points(lon1, lat1, 1)
        c2, r2 = n2.transform_points(lon2, lat2, 1)
        du_pix = c2 - c1
        dv_pix = r2 - r1
        return c1, r1, du_pix, dv_pix

    def _get_drift_PM_for_pol(self, pol,
                              grid_step_pix=40,
                              img_size=40,
                              min_border=40,
                              max_border=80,
                              ...
                              **kwargs):
        """
        Run PM on one polarization, using FT for that same polarization.
        Returns:
        ...
        lon_pm1, lat_pm1 = n1.transform_points(pm_cols, pm_rows, 0)

        # FT on this polarization
        c1_ft, r1_ft, du_ft, dv_ft = self._get_drift_FT_for_pol(pol, **kwargs)
        if len(c1_ft) == 0:
            # No FT vectors; PM can't run meaningfully
            shape = pm_cols.shape
            nan = np.zeros(shape) + np.nan
            return nan, nan, nan, nan, nan, pm_cols, pm_rows

        # convert FT to synthetic coords
        lon1_ft, lat1_ft = n1.transform_points(c1_ft, r1_ft, 0)
        lon2_ft, lat2_ft = n2.transform_points(
            c1_ft + du_ft, r1_ft + dv_ft, 0
        )
        # convert back to "pixel" coords for PM interface
        x1_ft, y1_ft = n1.transform_points(lon1_ft, lat1_ft, 1)
        x2_ft, y2_ft = n2.transform_points(lon2_ft, lat2_ft, 1)
        ...
        xpm1, ypm1 = n1.transform_points(pm_cols, pm_rows, 0)
        xpm2 = xpm1 + u_syn
        ypm2 = ypm1 + v_syn
        cpm2, rpm2 = n1.transform_points(xpm2, ypm2, 1)
        u_pix = cpm2 - pm_cols
        v_pix = rpm2 - pm_rows
    ...
    def get_drift_FT(self, max_speed=0.5, max_drift=8000.0,
        ...
        """
        Run Feature Tracking (FT).

        If pol_mode is:
            "HV"     - FT on HV only (original behaviour)
            "HH"     - FT on HH only
            "HV+HH"  - FT on HV and HH separately, then concatenate vectors
        """
        if self.pol_mode in ["HV", "HH"]:
            return self._get_drift_FT_for_pol(
                self.pol_mode,
                max_speed=max_speed,
                max_drift=max_drift,
                ratio_test=ratio_test,
        ...
        elif self.pol_mode == "HV+HH":
            res = []
            for pol in ["HV", "HH"]:
                c1, r1, du, dv = self._get_drift_FT_for_pol(
                    pol,
                    max_speed=max_speed,
                    max_drift=max_drift,
                    ratio_test=ratio_test,
                ...
                if len(c1) > 0:
                    res.append((c1, r1, du, dv))

            if len(res) == 0:
                return np.array([]), np.array([]), np.array([]), np.array([])

            c1 = np.concatenate([r[0] for r in res])
            r1 = np.concatenate([r[1] for r in res])
            du = np.concatenate([r[2] for r in res])
            dv = np.concatenate([r[3] for r in res])
            return c1, r1, du, dv

        else:
            raise ValueError("pol_mode must be 'HV', 'HH' or 'HV+HH'")
    ...
    def get_drift_PM(self, grid_step_pix=40,
                     ...
                     **kwargs):
        """
        Run Pattern Matching (PM).

        If pol_mode is:
            "HV"     - PM on HV only (original behaviour)
            "HH"     - PM on HH only
            "HV+HH"  - PM on HV and HH separately, then combine:

                         score_pol = r_pol * h_pol
                         choose pol with max(score_pol) at each grid point

        Returns:
            u_pix, v_pix, a_deg, r_mcc, h_hess, pm_cols, pm_rows
        """
        if self.pol_mode in ["HV", "HH"]:
            return self._get_drift_PM_for_pol(
                self.pol_mode,
            ...
            # Run PM separately on HV and HH
            u_HV, v_HV, a_HV, r_HV, h_HV, pm_cols, pm_rows = self._get_drift_PM_for_pol(
                "HV",
                grid_step_pix=grid_step_pix,
                img_size=img_size,
                min_border=min_border,
            ...
            u_HH, v_HH, a_HH, r_HH, h_HH, pm_cols2, pm_rows2 = self._get_drift_PM_for_pol(
                "HH",
                grid_step_pix=grid_step_pix,
                img_size=img_size,
                min_border=min_border,
            ...
            # assume same grid for HV and HH
            # combine using score = r * h, but do NOT threshold here
            score_HV = r_HV * h_HV
            score_HH = r_HH * h_HH

            # treat NaNs as -inf so they are never chosen if other is valid
            score_HV_f = np.where(np.isfinite(score_HV), score_HV, -np.inf)
            score_HH_f = np.where(np.isfinite(score_HH), score_HH, -np.inf)

            use_HH = score_HH_f > score_HV_f
            best_score = np.where(use_HH, score_HH_f, score_HV_f)

            # start from HV and overwrite where HH wins
            u_comb = np.where(use_HH, u_HH, u_HV)
            v_comb = np.where(use_HH, v_HH, v_HV)
            a_comb = np.where(use_HH, a_HH, a_HV)
            r_comb = np.where(use_HH, r_HH, r_HV)
            h_comb = np.where(use_HH, h_HH, h_HV)

            # where both are invalid, best_score == -inf -> set all to NaN
            invalid = ~np.isfinite(best_score) | (best_score == -np.inf)
            u_comb[invalid] = np.nan
            v_comb[invalid] = np.nan
            a_comb[invalid] = np.nan
            r_comb[invalid] = np.nan
            h_comb[invalid] = np.nan

            return u_comb, v_comb, a_comb, r_comb, h_comb, pm_cols, pm_rows
    ...
    def interpolate_to_dense_image_grid(self, pm_cols, pm_rows, u, v, good_mask,
        ...
        good_mask : 2D bool array
            Quality mask (e.g., (r*h) > threshold).
        img_shape : tuple
        ...
        if np.sum(mask_flat) == 0:
            # no valid points
            return (np.zeros(img_shape) + np.nan,
                    np.zeros(img_shape) + np.nan)
...
def warp_image_with_flow_anchor(img, u, v, anchor_dy=0.0, anchor_dx=0.0,
                         order=1, mode='constant', cval=np.nan):
    """
    Backward warp, but evaluate the flow at shifted locations:
      use flow at (r+anchor_dy, c+anchor_dx) instead of (r,c).
    Positive anchor_dy means "apply flow from a bit lower in the image".
    """
    rows, cols = img.shape
    rr, cc = np.meshgrid(np.arange(rows), np.arange(cols), indexing='ij')

    # sample flow at shifted anchor locations (subpixel allowed)
    u_a = map_coordinates(u.astype(np.float64), [rr + anchor_dy, cc + anchor_dx],
                          order=1, mode='nearest')
    v_a = map_coordinates(v.astype(np.float64), [rr + anchor_dy, cc + anchor_dx],
                          order=1, mode='nearest')

    # backward mapping with anchored flow
    r_src = rr - v_a
    c_src = cc - u_a

    warped = map_coordinates(img.astype(np.float64), [r_src, c_src],
