...
def sar_t_from_future_npz(future_path: str) -> Optional[str]:
    """
    Read SAR path at time t from the future drift NPZ.
    For future drift: [t, t+24] -> meta['start_path'] is the SAR at time t.
    """
    try:
...
def build_split_indices_jsonl_all_regions(
    ...
    check_sar_t_matches_t: bool = False,  # optional safety
):
    ...
    for (region, t) in keys:
        ...
        if add_sar_t_path:
            ...
            if check_sar_t_matches_t:
                sar_t = t_from_wind(sar_t_path) or (TS_RE.search(Path(sar_t_path).name).group(1) if TS_RE.search(Path(sar_t_path).name) else None)
                if sar_t is not None and sar_t != t:
                    skipped["sar_t_mismatch"] += 1
                    continue
...
drift_root = os.environ.get("DRIFT_ROOT", "path/to/VECTOR_FIELDS_24h_pairs_wBackwardPastDrift_velocity/HV")
...
build_split_indices_jsonl_all_regions(
    ...
    out_train_jsonl="model_dev_main/index_files/full_wind_drift_SAR_dataset_wBackwardPastDrift/index_train.jsonl",
    out_val_jsonl="model_dev_main/index_files/full_wind_drift_SAR_dataset_wBackwardPastDrift/index_val.jsonl",
    out_test_jsonl="model_dev_main/index_files/full_wind_drift_SAR_dataset_wBackwardPastDrift/index_test.jsonl",
