...
def main():
    ...
    # SAR defaults (match your training defaults)
    ...
    if in_ch != inferred_in_ch:
        raise ValueError(f"in_ch={in_ch} but dataset provides {inferred_in_ch} channels. "
    ...
    for idx in picks:
        ...
        past_u = denorm_x_channel(x, "past_drift_u", i_past_u, inputs_stats, device).cpu().numpy()
        past_v = denorm_x_channel(x, "past_drift_v", i_past_v, inputs_stats, device).cpu().numpy()
        wind_u = denorm_x_channel(x, "future_wind_u10_mean", i_wind_u, inputs_stats, device).cpu().numpy()
        wind_v = denorm_x_channel(x, "future_wind_v10_mean", i_wind_v, inputs_stats, device).cpu().numpy()
        ...
        # Shared drift range WITHIN this figure (past/true/pred)
        ...
        # Each panel has its own colorbar, but drift ones share the same norm
